...
contract VESPLXToken is ERC721 {
    ...
    constructor(address _protocolToken) ERC721("Split Voting Escrow", "veSPLX") {
        deployer = msg.sender;
        protocolToken = _protocolToken;
        pictureUri = "https://raw.githubusercontent.com/SplitProtocol/nft/main/Split_NFT.png";
        diffTokens = false;
    ...
    function depositCustomLock(uint256 amount, uint256 multiplierTSID, uint256 lockTime, address token) external onlyOwner {
        require(possibleEscrowToken[token], "Split: token not allowed for staking");
        require(amount > 0, "Split: zero amount");
        require(multiplierTSID <= 3, "Split: bad id");
        uint256 tokenAmount;
        ...
        uint256 veShare = lockTimeMultiplier[multiplierTSID] * tokenAmount / 100;
        uint256 unlockTimestamp = lockTime * ONE_MONTH + block.timestamp;
